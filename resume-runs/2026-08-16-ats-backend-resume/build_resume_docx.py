...
def add_hyperlink(paragraph, text: str, url: str, color: str = MUTED) -> None:
...
def add_header(doc: Document, data: dict[str, Any], profile: str) -> None:
    ...
    profile_items = [
        (key, str(contact.get(key, "")).strip(), str(contact.get(f"{key}_url", "")).strip())
    ...
    if profile_items:
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER if profile == "ats-safe" else WD_ALIGN_PARAGRAPH.LEFT
        for index, (_, display, url) in enumerate(profile_items):
            if index:
                separator = p.add_run(" | ")
                set_run(separator, size=9.5, color=MUTED)
            add_hyperlink(p, display, url or f"https://{display}")
        p.paragraph_format.space_after = Pt(8)
